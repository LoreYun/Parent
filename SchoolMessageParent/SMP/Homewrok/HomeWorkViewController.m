...
-(void)getBulletins:(void(^)(BOOL succeed,NSDictionary *result))callback;
{
    ...
    NSDictionary *dic = @{@"1":classId,@"2":studentId,@"3":@"15",@"4":[NSString stringWithFormat:@"%ld",(long)self.page]};
    [[ObjectManager sharedInstance] requestDataOnPost:dic ByFlag:@"21503" callback:callback];
}


-(void)isRead:(NSString *)hId callback:(void(^)(BOOL succeed,NSDictionary *result))callback;
{
    ...
    NSString *studentId = [[AccountManager sharedInstance].LoginInfos getStudentId];
    NSString *studentName = [[AccountManager sharedInstance].LoginInfos getStudentName];
    NSDictionary *dic = @{@"1":hId,@"2":studentId,@"3":studentName,@"4":pid,@"5":pname,@"6":@"15",@"7":[NSString stringWithFormat:@"%ld",(long)self.page]};
    [[ObjectManager sharedInstance] requestDataOnPost:dic ByFlag:@"21502" callback:callback];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (readFlag.intValue !=1) {
        [self isRead:[dic objectForKey:@"HomeworkId"] callback:^(BOOL succeed, NSDictionary *result) {
            
        }];
    }
    
    [dic setValue:[NSNumber numberWithInt:1] forKey:@"RealFlag"];
    [self.tableView reloadData];
