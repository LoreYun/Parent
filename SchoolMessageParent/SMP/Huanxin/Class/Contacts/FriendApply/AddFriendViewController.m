...
@interface AddFriendViewController ()<UITextFieldDelegate, UIAlertViewDelegate,MBProgressHUDDelegate>
{
    MBProgressHUD *HUD;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",photoUrl,[[self.dataSource objectAtIndex:indexPath.row] valueForKey:S_TOUXIANG]]] placeholderImage:[UtilManager imageNamed:@"chatListCellHead"]];
    cell.textLabel.text = [[self.dataSource objectAtIndex:indexPath.row] valueForKey:S_NICKNAME];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *buddyName = [[self.dataSource objectAtIndex:indexPath.row] valueForKey:S_ACCOUNT];
    NSString *nickName = [[self.dataSource objectAtIndex:indexPath.row] valueForKey:S_NICKNAME];
    if ([self didBuddyExist:buddyName]) {
        NSString *message = [NSString stringWithFormat:@"'%@'已经是你的好友了!", nickName];
    ...
    {
        NSString *message = [NSString stringWithFormat:@"您已向'%@'发送好友请求了!", nickName];
        [WCAlertView showAlertWithTitle:message
                                message:nil
                     customizationBlock:nil
                        completionBlock:nil
...
#warning 以下代码为测试代码，默认用户体系中有一个符合要求的同名用户
        ...
        if([r currentReachabilityStatus]==NotReachable)
        {
            ...
            HUD.delegate = self;
            HUD.labelText = @"当前网络未连接";
            [HUD show:YES];
            [HUD hide:YES afterDelay:1.2];
        }else
        {
            [MBProgressHUD showHUDAddedTo:self.view animated:YES];
            [[RosterManager sharedInstance] getPersonInfoByNickName:_textField.text callback:^(BOOL succeed, NSDictionary *result) {
                [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                NSLog(@"%@",result);
                NSArray *arr = [result objectForKey:S_OBJ];
                if (arr.count==0) {
                    [self showHint:@"所搜的好友不存在"];
                }
...
-(void)refreshUI:(NSMutableDictionary *)dic
{
    [self.dataSource removeAllObjects];
    
    [self.dataSource addObjectsFromArray:[dic valueForKey:S_OBJ]];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if ([alertView cancelButtonIndex] != buttonIndex) {
        ...
        NSString *messageStr = @"";
        NSString *username = [RosterManager sharedInstance].myInfo.nickName;
