...
@property (nonatomic) NSInteger friendTop;
...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setCellForRowAtIndexPathCompletion:^UITableViewCell *(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            {
                [cell.imageView setImageWithURL:[NSURL URLWithString:info.friendInfo.headPhotoUrl] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]];
            }
            cell.textLabel.text =info.friendInfo?info.friendInfo.nickName:info.buddy.username; //buddy.username;
        ...
        [_searchController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            if (loginUsername && loginUsername.length > 0) {
                if ([loginUsername isEqualToString:info.buddy.username]) {
            ...
            ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:info.buddy.username isGroup:NO];
            chatVC.title = info.buddy.username;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        if (indexPath.section == 0 && indexPath.row == 1) {
            ...
            BuddyInfo *info = [[self.dataSource objectAtIndex:(indexPath.section - 1)] objectAtIndex:indexPath.row];
            if (!info.friendInfo)
            {
                cell.imageView.image =[UIImage imageNamed:@"chatListCellHead.png"];
            }else
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self.contactsSource removeObject:info];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            for (NSInteger i = ([RosterManager sharedInstance].rosterData.count-1); i>=0; i--) {
                EMBuddy * bd = [[RosterManager sharedInstance].rosterData objectAtIndex:i];
                if ([bd.username isEqualToString:buddy.username]) {
                    [[RosterManager sharedInstance].rosterData removeObject:bd];
                    break;
                }
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:info.buddy.username isGroup:NO];
        chatVC.title =  info.friendInfo?info.friendInfo.nickName:info.buddy.username;
...
{
    [[RealtimeSearchUtil currentUtil] realtimeSearchWithSource:self.contactsSource searchText:(NSString *)searchText collationStringSelector:@selector(getNickName) resultBlock:^(NSArray *results) {
        if (results) {
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex != actionSheet.cancelButtonIndex && _currentLongPressIndex) {
        BuddyInfo *info = [[self.dataSource objectAtIndex:(_currentLongPressIndex.section - 1)] objectAtIndex:_currentLongPressIndex.row];
        EMBuddy *buddy = info.buddy;
...
- (void)cellImageViewLongPressAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *loginUsername = [loginInfo objectForKey:kSDKUsername];
    BuddyInfo *info = [[self.dataSource objectAtIndex:(indexPath.section - 1)] objectAtIndex:indexPath.row];
    if ([info.buddy.username isEqualToString:loginUsername])
    {
        return;
    }
...
- (NSMutableArray *)sortDataArray:(NSArray *)dataArray
{
    ...
    //名字分section
    for (BuddyInfo *info in dataArray) {
        //getUserName是实现中文拼音检索的核心，见NameIndex类
        NSString *firstLetter = [ChineseToPinyin pinyinFromChineseString:[info getNickName]];
    ...
    for (int i = 0; i < [sortedArray count]; i++) {
        NSArray *array = [[sortedArray objectAtIndex:i] sortedArrayUsingComparator:^NSComparisonResult(BuddyInfo *obj1, BuddyInfo *obj2) {
...
- (void)reloadDataSource
{
    ...
    //为了修正buddylist 偶尔count为0的情况，暂行办法
    NSArray *buddyList = [[EaseMob sharedInstance].chatManager buddyList];NSLog(@"buddyList.COUNT IS %d",buddyList.count);
    if (![RosterManager sharedInstance].rosterData) {
        [RosterManager sharedInstance].rosterData = [NSMutableArray arrayWithArray:buddyList];
//        [[RosterManager sharedInstance].rosterData addObjectsFromArray:buddyList];
    }else
    {
        if (buddyList.count ==0 && buddyList.count!= ([RosterManager sharedInstance].rosterData.count-1)) {
            buddyList =  [RosterManager sharedInstance].rosterData;
        }else
        {
            [[RosterManager sharedInstance].rosterData removeAllObjects];
            [[RosterManager sharedInstance].rosterData addObjectsFromArray:buddyList];
        }
    }
    __weak ContactsViewController * controller = self;
    self.friendTop = buddyList.count;
...
//    [self.contactsSource removeAllObjects];
    ...
    [[RosterManager sharedInstance] getFriendInfoByChatterArray:arr callBack:^(BOOL succeed, NSArray *firendArray) {
        ...
        [controller hideHud];
        [controller.slimeView endRefresh];
    }];
}
