...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate,InPutMessage,SearchGroupListDelegate>
{
...
@property (strong, nonatomic) NSMutableArray *dataSource;//tableView数据源 小七
...
//单人会话信息
@property (strong, nonatomic) FriendInfo *friendInfo;
...
- (instancetype)initWithChatter:(NSString *)chatter isGroup:(BOOL)isGroup
{
    ...
    if (self) {
        ...
        if(!_isChatGroup)
        {
            ...
            [[RosterManager sharedInstance] getFriendInfoByChatter:self.chatter callBack:^(BOOL succeed, FriendInfo *firendInfo) {
                vc.friendInfo = firendInfo;
            }];
        }
...
- (void)viewDidLoad
{
    ...
    NSLog(@"head my:%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"touxiangurl"]);
    NSLog(@"head my:%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"studentNum"]);
    
    if (!_isChatGroup) {
        [[RosterManager sharedInstance] getFriendInfoByChatter:self.chatter callBack:^(BOOL succeed, FriendInfo *firendInfo) {
            if (succeed) {
                self.title = firendInfo.nickName;
            }
        }];
    }
...
- (DXMessageToolBar *)chatToolBar
{
    if (_chatToolBar == nil) {
        ...
        _chatToolBar.messageDelegate = self;
        _chatToolBar.isChatGroup =_isChatGroup;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if(model.isSender)
            {
                model.headImageURL = [NSURL URLWithString:[RosterManager sharedInstance].myInfo.headPhotoUrl];
            }else
            {
                ...
                {
                    if (self.friendInfo) {
                        model.headImageURL = [NSURL URLWithString:self.friendInfo.headPhotoUrl];
                    }
...
-(NSDictionary *)getExt
{
    NSString *head = [[AccountManager sharedInstance].LoginInfos getHeadUrl];
    NSString *name = [[AccountManager sharedInstance].LoginInfos getNickName].length>0?[[AccountManager sharedInstance].LoginInfos getNickName]:@"";
    
    return @{@"renyuannicheng":name,@"renyuantouxiang":head};
}
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
    ...
    }else if([eventName isEqualToString:kRouterEventChatHeadImageTapEventName]&&model.isChatGroup&&!model.isSender)
    {

        ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:model.message.groupSenderName isGroup:NO];
...
{
    if (_isChatGroup && [_conversation.chatter isEqualToString:group.groupId]) {
...
//                [messages addObject:prevMessage];
                pre = _longPressIndexPath.row-1;
//                 [self.dataSource removeObjectAtIndex:_longPressIndexPath.row-1];
//                [indexPaths insertObject:[NSIndexPath indexPathForRow:(_longPressIndexPath.row - 1) inSection:0] atIndex:0];
            }
        }
...
//        [self.tableView deleteRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationFade];
        ...
        if (pre<NSIntegerMax) {
            [self.dataSource removeObjectAtIndex:pre];
        }
...
- (void)loadMoreMessages
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        if (latestMessage && (!chats || chats.count==0)) {
            chats = [weakSelf.conversation loadNumbersOfMessages:(currentCount + KPageCount) before:([[NSDate date] timeIntervalSince1970] * 1000 + 1)];
        }
        
        if (latestMessage && (!chats || chats.count==0)) {
            chats = [weakSelf.conversation loadAllMessages];
        }
...
{
    if (_isChatGroup) {
        SearchGroupListViewController *detailController = [[SearchGroupListViewController alloc] initWithGroupId:_chatter];
        detailController.title = self.title;
...
{
    self.chatToolBar.inputTextView.text = [NSString stringWithFormat:@"%@%@ ",self.chatToolBar.inputTextView.text,text];
}
