...

{
    ...
    if ([_blockSelectedUsernames count] > 0) {
        ...
        return ![self isBlockUsername:buddy.friendInfo.nickName.length>0?buddy.friendInfo.nickName:buddy.friendInfo.chatter];
...
{
    [[RealtimeSearchUtil currentUtil] realtimeSearchWithSource:self.contactsSource searchText:searchText collationStringSelector:@selector(getNickName) resultBlock:^(NSArray *results) {
