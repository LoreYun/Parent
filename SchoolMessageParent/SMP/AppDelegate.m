...
#define kAppId           @"PcywdZypTl82fU3wQigmT3"
#define kAppKey          @"xIBmQWRH08AF0w50tDec97"
#define kAppSecret       @"ot5567I4Ob69jNTvwovAs1"
...
- (void)GexinSdkDidReceivePayload:(NSString *)payloadId fromApplication:(NSString *)appId
{
    ...
    if (payload) {
        payloadMsg = [[NSString alloc] initWithBytes:payload.bytes
                                            ...
                                            encoding:NSUTF8StringEncoding];//校园公告@`校园公告@`测试校园公告推送1
...
-(void)pushVCByData:(NSString *)payloadMsg
{
    ...
    if([payloadMsg isEqualToString:@"校园公告"])
    {
        BulletinViewController *vc = [BulletinViewController new];
        [root pushViewController:vc animated:YES];
    }else if ([payloadMsg isEqualToString:@"学生点评"])
    {
        EvaluationViewController *vc = [EvaluationViewController new];
        [root pushViewController:vc animated:YES];
    }else if ([payloadMsg isEqualToString:@"班级通知"])
    {
        NotificationViewController *vc = [NotificationViewController new];
        [root pushViewController:vc animated:YES];
    }else if ([payloadMsg isEqualToString:@"考勤通知"])
    {
        AttdendanceViewController *vc = [AttdendanceViewController new];
        [root pushViewController:vc animated:YES];
    }
}
...
{
    return ([payloadMsg isEqualToString:@"学生点评"] || [payloadMsg isEqualToString:@"班级通知"]||[payloadMsg isEqualToString:@"考勤通知"]|| [payloadMsg isEqualToString:@"校园公告"]);
}
