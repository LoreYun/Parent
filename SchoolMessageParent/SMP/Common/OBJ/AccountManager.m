...
-(void)login:(NSString *)username pwd:(NSString *)pwd callback:(void (^)(BOOL, NSString *))callback
{
    ...
    [self getCloudConfig:username callback:^(BOOL s, NSDictionary *result) {
        if (s) {
            ...
            [self requestDataOnGet:accdic ByFlag:@"2100" callback:^(BOOL success, NSDictionary *data) {
                if (success) {
                    self.LoginInfos = [[LoginInfo alloc] initByLogin:[data objectForKey:@"Obj"]];
                    
                    [self setPushTag:data];
                    [self uploadClientId:[self.LoginInfos getParentsId]];
                ...
                callback(success,error.length>0?error:@"操作失败，请检查网络");
            }];
        }else
        {
            NSString *error = [result objectForKey:@"Msg"];
            callback(NO,error.length>0?error:@"操作失败，请检查网络");
        }
    }];
}
...
-(void)uploadClientId:(NSString *)accountId
{
    ...
    if (appDelegate.gexinPusher) {
        ...
        if (ccc.length>0) {
            ...
            [self requestDataOnPost:dic ByFlag:@"2101" callback:^(BOOL result, NSDictionary *data) {
                if (result) {
