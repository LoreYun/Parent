...
-(void)setCellView
{
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(15,136,SCREEN_WIDTH-30,206)];
    view.backgroundColor = [UIColor whiteColor];
    view.layer.cornerRadius = 6;
    view.layer.masksToBounds = YES;
    [self.view addSubview:view];
    ...
    UIView *cell2 = [self createCell:@"shezhi" title:@"我的课堂" startY:54 action:@selector(push2Class:)];
    ...
    UIView *cell3 = [self createCell:@"shezhi" title:@"设置" startY:108 action:@selector(push2Setting:)];
    
    [view addSubview:cell3];
    
    UIView *div2 = [[UIView alloc] initWithFrame:CGRectMake(4,161.5,SCREEN_WIDTH-30-8,0.5)];
    div2.backgroundColor = [UtilManager getColorWithHexString:@"4b4b4b"];
    [view addSubview:div2];
    
    UIView *cell4 = [self createCell:@"bangzu" title:@"帮助与反馈" startY:162 action:@selector(push2Help:)];
...
- (void)upload {
    ...
    [[ObjectManager sharedInstance] uploadUserImage2Server:data callback:^(BOOL succeed, NSDictionary *data) {
        if (succeed) {
...
{
    [[ObjectManager sharedInstance] requestDataOnPost:@{@"1":[[AccountManager sharedInstance].LoginInfos getParentsId],@"2":touxiangUrl} ByFlag:@"2703" callback:^(BOOL succeed, NSDictionary * dic) {
        if (succeed) {
