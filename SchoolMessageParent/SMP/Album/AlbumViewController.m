...
//    [backBtn setImage:[UIImage imageNamed:@"add"] forState:UIControlStateNormal];
...
//    
//    titleLabel.text = [[AccountManager sharedInstance].LoginInfos getClassName];
...
//    self.navigationItem.titleView = titleLabel;
    self.title = [[AccountManager sharedInstance].LoginInfos getClassName];
}
...
//    NSString *idold = ((NSNumber *)[data objectForKey:@"ClassId"]).stringValue;
...
-(void)creatRefreshHeader:(UIScrollView *)scr
{
    ...
    {
        weak.page = 1;
        [[AlbumManager sharedInstance] getAlbumList:weak.page classId:[[AccountManager sharedInstance].LoginInfos getClassId] gradeId:[[AccountManager sharedInstance].LoginInfos getGradeId] callback:^(BOOL succeed, NSDictionary *data) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.publishLabel.text = [time componentsSeparatedByString:@" "].firstObject;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableDictionary *dic =[mainDataSource objectAtIndex:indexPath.row];
    AlbumDetailViewController *albumDetail = [[AlbumDetailViewController alloc] initWithAlbumId:[dic objectForKey:@"AlbumId"] AlbumName:[dic objectForKey:@"AlbumName"] classInfos:nil];
