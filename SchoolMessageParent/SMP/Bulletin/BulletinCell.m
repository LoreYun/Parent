...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.title = [[UILabel alloc] initWithFrame:CGRectMake(25, 37, 173, 19)];
        self.title.numberOfLines = 1;
        self.title.font = [UIFont systemFontOfSize:18];
        [self addSubview:self.title];
        
        self.content = [[UILabel alloc] initWithFrame:CGRectMake(25, 63, self.frame.size.width-50, 32)];
        self.content.numberOfLines = 2;
        self.content.font = [UIFont systemFontOfSize:15];
        [self addSubview:self.content];
        
        self.creator= [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width-120-25,37, 120, 16)];
        ...
        self.timeLabel= [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width-100-25, 23, 100, 16)];
...
{
    self.title.text = [dic objectForKey:@"NoticeTitle"];
    self.content.text = [dic objectForKey:@"GaiYao"];
    self.creator.text = [NSString stringWithFormat:@"发起人：%@",[dic objectForKey:@"FaQiRenName"]];
    ...
    self.content.frame = CGRectMake(25, 63, size.width, size.height);
    self.timeLabel.frame = CGRectMake(SCREEN_WIDTH-150-25, CGRectGetMaxY(self.content.frame)+8, 150, 16);
    self.bgView.image = [[UtilManager imageNamed:@"bulletin_new_bg"] stretchableImageWithLeftCapWidth:30 topCapHeight:50];
    self.bgView.frame = CGRectMake(0,10,SCREEN_WIDTH,size.height+63+32);
}
